...
public class Audio
{
     /// <summary>
        ...
        public Transform SourceTransform
        {
            ...
            set { sourceTransform = value == null ? AudioManager.Instance.transform : value; }
        }
        ...
        public Audio(AudioType audioType, AudioClip clip, bool loop, bool persist, float volume, float fadeInValue, float fadeOutValue, Transform sourceTransform)
        {
            ...
            Pitch = 1;
            StereoPan = 0;
            if (sourceTransform != null && sourceTransform != AudioManager.Instance.transform)
            {
                SpatialBlend = 1;
            }
            ReverbZoneMix = 1;
        ...
        public void Play(float volume)
        {
            ...
            if (Pooled)
            {
                ...
                bool restoredFromPool = AudioManager.RestoreAudioFromPool(Type, AudioID);
                if (!restoredFromPool)
                {
                    return;
                }
        ...
        public void Update()
        {
            ...
            switch (Type)
            {
                case AudioType.Music:
                {
                    AudioSource.volume = Volume * AudioManager.Instance.GetMusicVolume();
                        break;
                    }
                case AudioType.Sound:
                {
                    AudioSource.volume = Volume * AudioManager.Instance.GetSoundVolume();
                        break;
                    }
                case AudioType.UISound:
                    {
